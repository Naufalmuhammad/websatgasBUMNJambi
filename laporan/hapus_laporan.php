...
if(is_file("files/".$data['file'])) // Jika foto ada
  unlink("files/".$data['file']); // Hapus foto yang telah diupload dari folder images

// Query untuk menghapus data siswa berdasarkan ID yang dikirim
$sql = $pdo->prepare("DELETE FROM laporan WHERE id=:id");
$sql->bindParam(':id', $id);
...
if($execute){ // Cek jika proses simpan ke database sukses atau tidak
  ...
  // Jika Gagal, Lakukan :
  echo "Data gagal dihapus. <a href='indexLaporan.php'>Kembali</a>";
}
?> 